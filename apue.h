...
#define _POSIX_ C_SOURCE 200809L
...
#define  MARLINE  9096                      /* max line length */

/*
 * Default file access permissions for new files. 
 */

 #define  FILE_MODE     (S_IAUSR | S_IWUSR | S_IRGRP | S_IROTH) 
...
void      pr_mask(const char*)                    /*Figure 10.19*/                           
Sigfunc   *signal intr(int, Sigfunc*);            /*Figure 10.19*/

void      daemonize(const char*);                 /* Figure 13.1*/

void      sleep_us(unsigned int);                 /*Exercise 19.5*/
ssize t   readn(int, void*，size_t);              /*Figure 14.24*/
ssize t   writen(int, const void*, sizet);        /*Figure 14.29*/

int       fd_pipe (int*);                         /*Figure 17.2*/
int       recv_fd(int, ssize_t (*func)(int, 
                    const void *, size_t));       /* Fiaure 17.14*/
...
#define writew_lock(fd, offset, whence, len) \
          lock_reg((fd), F_SETLKW, F_ERLCK, (offset), (whence), (len))
